...
public class AuthFilter implements Filter {
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        System.out.println("[AuthFilter] Filtering URI: " + uri);

        boolean loggedIn = (session != null && session.getAttribute("user") != null);

        if (uri.contains("View/adminDashboard.jsp")) {
            if (loggedIn && ((model.User)session.getAttribute("user")).getRole().equals("admin")) {
                System.out.println("[AuthFilter] Access allowed to adminDashboard.jsp");
                chain.doFilter(req, res);
            } else {
                System.out.println("[AuthFilter] Access denied to adminDashboard.jsp");
            ...
            if (loggedIn) {
                System.out.println("[AuthFilter] Access allowed to userDashboard.jsp");
                chain.doFilter(req, res);
            } else {
                System.out.println("[AuthFilter] Access denied to userDashboard.jsp");
